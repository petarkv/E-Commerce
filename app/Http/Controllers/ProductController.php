...
class ProductController extends Controller
{
    ...
    public function addtocart(Request $request){
        ...
        if(!empty($data['wishListButton']) && $data['wishListButton']=="Add to Wish List"){        
            // echo "Wish List is selected"; die;

            // Check User is logged in
            if (!Auth::check()) {
                return redirect()->back()->with('flash_message_error','Please login to add product
                    in Your Wish List');
            }
            // Check Size is selected
            if(empty($data['size'])){
                return redirect()->back()->with('flash_message_error','Please select size to add product
                    in Your Wish List'); 
            }
    ...
    public function wishList(){
        ...
        $meta_title = "Wish List - MyShop";
        $meta_description = "View Wish List of MyShop Website";
        $meta_keywords = "wish list, ecommerce website, myshop website";
        ...
        footer {
          ...
          <body>
            <header class="clearfix">
              <div id="logo">
                <img src="images/template_images/my_logo.jpg">
              </div>
              ...
              <div id="project" style="float:right;">
                ...
                <div><span>Name/Surname</span>' .$orderDetails->name.' '.$orderDetails->surname.'</div>
                <div><span>Address</span>' .$orderDetails->address.'</div>
                <div><span>City/State</span>' .$orderDetails->city.', '.$orderDetails->state.'</div>
                <div><span>Postal Code</span>' .$orderDetails->pincode.'</div>
              ...
              <table>
                ...
                foreach($orderDetails->orders as $pro){
                    $output .= '<tr>
                        ...
                        <td class="text-center">EUR '.$pro->product_price.'</td>
                        <td class="text-center">'.$pro->product_quantity.'</td>
                        <td class="text-right">EUR '.$pro->product_price * $pro->product_quantity.'</td>
                    </tr>';
                    $Subtotal = $Subtotal + ($pro->product_price * $pro->product_quantity); }
                $output .= '<tr>
                    <td colspan="5">SUBTOTAL</td>
                    <td class="total">EUR '.$Subtotal.'</td>
                  </tr>
                  <tr>
                    <td colspan="5">SHIPPING CHARGES (+)</td>
                    <td class="total">EUR '.$orderDetails->shipping_charges.'</td>
                  </tr>
                  <tr>
                    <td colspan="5">COUPON DISCOUNT (-)</td>
                    <td class="total">EUR '.$orderDetails->coupon_amount.'</td>
                  </tr>
                  <tr>
                    <td colspan="5" class="grand total">GRAND TOTAL</td>
                    <td class="grand total">EUR '.$orderDetails->grand_total.'</td>
                  </tr>
                </tbody>
              </table>
            </main>
    ...
    public function ipnPaypal(Request $request){
        ...
        if($data['payment_status']=="Completed"){
            ...
            Mail::send('emails.order',$messageData,function($message) use($email){
                $message->to($email)->subject('Order Placed - MyShop Website');    
            });
